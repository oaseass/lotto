...
export async function POST(req: NextRequest) {
  try {
    ...
    if (anthropic) {
      try {
        const prompt = buildReasonPrompt(cheonjigan, ohaeng, numbers, date, yongsin)
        ...
        reason = `오늘 일진과 ${yongsin} 기운이 강한 날로, 용신 오행의 수리가 반영된 번호입니다.`
      }
    } else {
      ...
      const yongsinNums = numbers.filter(n => numOhaeng(n) === yongsin).join(', ')
      const sangsaengNums = numbers.filter(n => numOhaeng(n) === sangsaeng).join(', ')
      reason = `오늘 일진(${ilja})과 용신 ${yongsin}(${OHAENG_HANJA[yongsin]}) 기운을 반영한 번호입니다. ` +
        (yongsinNums ? `${yongsinNums}은(는) 용신 ${yongsin} 오행 수리(끝자리 ${OHAENG_LUCKY[yongsin]})에 해당합니다. ` : '') +
        (sangsaengNums ? `${sangsaengNums}은(는) 상생 ${sangsaeng}(${OHAENG_HANJA[sangsaeng]}) 기운으로 용신을 보완합니다.` : '')
    }
